...
using mapper::V3_0::renesas::passthrough::grallocDecodeBufferDescriptor;

constexpr int reservedBitsOffset = 32;

// Gralloc0HalImpl implements V3_0::hal::AllocatorHal on top of gralloc0
class Gralloc0HalImpl : public hal::AllocatorHal {
public:
    ~Gralloc0HalImpl() {
        if (mDevice) {
    ...
    Error allocateBuffers(const BufferDescriptor& descriptor, uint32_t count, uint32_t* outStride,
        ...
        mapper::V3_0::IMapper::BufferDescriptorInfo descriptorInfo;
...
protected:
    Error allocateOneBuffer(const mapper::V3_0::IMapper::BufferDescriptorInfo& info,
                            const native_handle_t** outBuffer, uint32_t* outStride) {
        if (info.layerCount > 1 || (info.usage >> reservedBitsOffset) != 0) {
