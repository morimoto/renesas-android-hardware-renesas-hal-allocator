...
// Gralloc1HalImpl implements V3_0::hal::AllocatorHal on top of gralloc1
class Gralloc1HalImpl : public hal::AllocatorHal {
public:
    ~Gralloc1HalImpl() {
        if (mDevice) {
    ...
    Error allocateBuffers(const BufferDescriptor& descriptor, uint32_t count, uint32_t* outStride,
        ...
        uint32_t stride = 0;
        uint32_t tmpStride = 0;
        const native_handle_t* tmpBuffer = nullptr;
...
protected:
    ...
    Error createDescriptor(const mapper::V3_0::IMapper::BufferDescriptorInfo& info,
